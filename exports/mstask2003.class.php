...
class MSTask2003 extends MSObject
{

  private $task = array();

  public function __construct($exporter, &$task)
  {
  ...
  public function dump()
  {
    $taskproperties = array(
      ...
      'Stop' => $this->dumpDateTime($this->task['task_updated']),
    ...
    if ($this->task['task_represents_project'] > 0) {
      ...
      $taskproperties['Project xmlns="http://schemas.microsoft.com/project"']
          = $exporter->exportProject($this->task['task_represents_project']);
    }
    return $taskproperties;
  }

  private function dumpPredecessorLinks()
  {
    $predecessorlinks = array();
    foreach ($this->task['task_predecessors'] as $task_predecessor){
      $predecessorlinks[] = array(
        "PredecessorLink" => array(
          "PredecessorUID" => $task_predecessor, 
          "Type" => 1,
          "CrossProject" => 0,
          "LinkLag" => 0,
          "LagFormat" => 7,
	...
	private function dumpTaskLogs()
	{
		$task = new CTask();
		$tasklogs = $task->getTaskLogs($this->task['task_id']);

		$timephaseddata = array();
		foreach ($tasklogs as $tasklog){
			$timephaseddata[] = array(
				"TimephasedData" => array(
					"Type" => 9,
					"UID" => $this->task['task_id'],
					"Start" => $tasklog['task_log_date'].'T'.w2PgetConfig('cal_day_start').':00:00',
					"Finish" => $tasklog['task_log_date'].'T'.w2PgetConfig('cal_day_end').':00:00',
					"Unit" => 1,
					"Value" => $tasklog['task_log_hours'],
				)
			);
		}
  ...
  private function dumpDurationDataTypeForTask($duration)
  {
    switch ($this->task['task_duration_type']) {
      case 1:
        return $duration ? "PT".$this->logToHours($duration) : "PT0H0M0S";
      default:
        $hoursperday = w2PgetConfig('cal_day_end') - w2PgetConfig('cal_day_start');
        return $duration ? "PT".$this->logToHours($duration * $hoursperday) : "PT0H0M0S";
    }
  }

  private function dumpTaskHoursWorked() 
  {
    return $this->task['task_hours_worked'] ? 
            "PT".$this->logToHours($this->task['task_hours_worked']) : 
            "PT0H0M0S";
  }

  private function dumpTaskDurationFormat()
  {
    return $this->task['task_duration_type'] == 1 ? 5 : 7;
  }
