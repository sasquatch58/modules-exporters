...
class MSProject2003 extends MSObject
{

  private $project = array();
  private $tasknum = 0;

  public function __construct($exporter, &$project)
  {
  ...
  public function dump()
  {
    return array(
      ...
      // Microsoft Office Project 2003 marks that task as a critical task.
      ...
      // Microsoft Office Project Server 2003 user or a Microsoft Windows NT user.
  ...
  private function dumpCalendars() 
  {
    $calendars = array();
  ...
  {
    $tasks = array();
    $this->dumpTasksTree($this->exporter->taskstree, $tasks);
    array_unshift($tasks, array('Task' => $this->dumpProjectSummaryTask()));
    return $tasks;
  }

  private function dumpTasksTree(&$taskstree, &$tasks)
  {
    foreach($taskstree as $tasknode) {
      $task = $tasknode['task'];
      if (is_null($task['task_created']) || 
          $task['task_created'] == "0000-00-00 00:00:00") {
        $task['task_created'] = $this->project['project_created'];
      }
      if (is_null($task['task_start_date']) || 
        $task['task_start_date'] == "0000-00-00 00:00:00") {
        $task['task_start_date'] = $this->project['project_start_date'];
      }
      $task['id'] = ++$this->tasknum;
      $mstask = $this->factory('Task', $task);
      $tasks[]= array('Task' => $mstask->dump());
      if (isset($tasknode['children'])) {
        $chidren = $tasknode['children'];
        $this->dumpTasksTree($chidren, $tasks);
      }
    }
  }

  private function dumpProjectSummaryTask()
  {
    $task = array (
      'task_id' => 0,
      'task_parent' => 0,
      'id' => 0,
      'task_name' => $this->project['project_name'],
      'task_created' => $this->project['project_created'],
      'task_start_date' => $this->project['project_start_date'],
      'task_end_date' => $this->project['project_end_date'],
      'task_owner_contact' => $this->project['owner'],
      'wbs' => 0,
      'wbs_level' => 0,
      'outline_number' => 0,
      'outline_level' => 0,
      'task_priority' =>  $this->project['project_priority'],
      'task_duration' => $this->project['project_duration'],
      'task_duration_type' => 1,
      'task_work' => $this->project['project_work'],
      'task_milestone' => 0,
      'task_summary' => 1,
      'task_percent_complete' => $this->project['project_percent_complete'],
      'task_target_budget' => $this->project['project_target_budget'],
      'task_actual_duration' => $this->project['project_duration'],
      'task_hours_worked' => $this->project['project_worked_hours'],
      'task_regular_work' => 0,
      'task_remaining_duration' => 0,
      'task_related_url' => $this->project['project_url'],
      'task_description' => $this->project['project_description'],
      'task_predecessors' => array(),
   );
   $mstask = $this->factory('Task', $task);
   return $mstask->dump();
  }

  private function dumpResources()
  {
    $resources = array();
    if (count($this->exporter->resources) > 0) {
      $id = 0;
      foreach ($this->exporter->resources as $resource){
        $resource['id'] = ++$id;
        $msresource = $this->factory('Resource', $resource);
        $resources[] = array('Resource' => $msresource->dump());
      }
    }
    return $resources;
  }

  private function dumpAssignments()
  {
    $assignments = array();
    if (count($this->exporter->assignments) > 0) {
      $uid = 0;
      foreach ($this->exporter->assignments as $assignment){
        $assignment['uid'] = ++$uid;
        $msassignment = $this->factory('Assignment', $assignment);
        $assignments[] = array( "Assignment" => $msassignment->dump());
      }
    }
    return $assignments;
  }
